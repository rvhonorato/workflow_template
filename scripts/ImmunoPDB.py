...
cysh = {
    ...
    "aho": 106,
    "wolfguy": 330,
}  # The position of the interface cysteine in each
cysl = {
    "kabat": 88,
    "chothia": 88,
    "martin": 88,
...
class PDBNumber(object):
    ...
    def numberChain(self, chain, sequence=""):
        ...
        if not sequence:  # No seqres provided
            ...
            which("muscle") or structureSequence in sequence
        ):  # Muscle found or it is an easy alignment
            chain.xtra["structure_ali"], chain.xtra["seqres_ali"] = pairwise_muscle(
                structureSequence, sequence
            )
            fix_deletions(
                chain
            )  # Fix the position of a missing residue if a degenerate alignment
            seqI2strucI = get_alignment_dict(
                chain.xtra["seqres_ali"], chain.xtra["structure_ali"]
            )
        else:  # Missing residues are suspected.
            raise Warning(
                "Muscle was not found and there are suspected missing residues. Numbering could be incorrect"
            )
            sequence = structureSequence
            seqI2strucI = get_alignment_dict(sequence, sequence)
        ...
        for numbering2Index, domainType, details in self.numberSequence(
            chain.id, sequence
        ):  # Iterate over the recognised domains.
            if domainType in self.allowed_domains:
                chain.xtra["domains"] = chain.xtra.get("domains", []) + [
                    domainType
                ]  # Append to the domains list.
                chain.xtra["anarci_details"] = chain.xtra.get("anarci_details", []) + [
                    details
                ]  # Append to the germlines list.
                for numberedId, index in numbering2Index:
                    if index not in seqI2strucI:
                        continue  # Missing residue
                    res = chain[index2ResId[seqI2strucI[index]]]
                    res.xtra[self.scheme] = numberedId  # Store the annotated numbering
                    if "pdb" not in res.xtra:
                        res.xtra["pdb"] = res.id[1:]  # Store the original numbering
                    res.xtra["domain_type"] = domainType  # Store the domain type
                    res.xtra[
                        "domain_index"
                    ] = di  # Store which domain this residue is in.

                # Second iteration to annotate non variable domains.
                if (
                    1 < details["query_start"] < 28
                ):  # If possible, annotate pre domain with 0C 0B 0A as pre insertions.
                    insertions = tebahpla[27 - details["query_start"] :]
                else:
                    insertions = " " * (
                        details["query_start"] + 1
                    )  # So that insertion code will always be at least ' ' # All
                ...
                for residue in chain:
                    ...
                    if pre:  # Update the insertion code
                        ...
                        ri += 1
                self.annotateChainRegions(
                    chain, self.scheme, self.region_definition, domain_index=di
                )
                di += 1
        ...
        if di and check_continuity:
            ...
            if not continuous:
                print(
                    "Warning: Numbering may be incorrect as a missing residue was detected in chain %s (around residue %d%s). Provide the seqres record to overcome this problem."
                    % (chain.id, chain.child_list[at].id[1], chain.child_list[at].id[2])
                )
    ...
    def annotateChainRegions(self, chain, scheme, definition=None, domain_index=0):
        ...
        """
        if definition is None or scheme == "pdb":
            return

        domain_type = chain.xtra["domains"][domain_index].replace("K", "L")
        numbering = [
            (r.xtra[scheme], None)
            if r.xtra.get("domain_index", None) == domain_index
            else ((0, " "), None)
            for r in chain
        ]
        ...
        try:
            regions = annotate_regions(
                numbering, domain_type, numbering_scheme=scheme, definition=definition
            )
    ...
    def numberSequence(self, sequenceName, sequence):
        ...
        for i in range(
            len(numbered)
        ):  # Iterate over the identified domains (e.g. for an scfv)
            numbering = [
                (n, a) for n, a in numbered[i][0] if a != "-"
            ]  # Remove gaps if made (imgt scheme)
            yield [
                (numbering[ri][0], ri + numbered[i][1]) for ri in range(len(numbering))
            ], details[i]["chain_type"], details[i]
...
class AntigenReceptorPDBParser(PDBParser):
    ...
    def __init__(
        ...
        """ """
        super(AntigenReceptorPDBParser, self).__init__(
    ...
    def get_structure(self, id, file):
        ...
        for chain in seqres:
            structure[0][chain].xtra["seqresobj"] = seqres[
                chain
            ]  # Associtate with the chain object - includes the original lines
        self.find_pairs(structure)  # Pair chains
        structure.switch_numbering_scheme(
            self.scheme
        )  # Return the structure with the requested numbering scheme.
        return structure
    ...
    def find_pairs(self, structure):
        ...
        for model in structure:
            ...
            model.xtra[
                "firstpair"
            ] = None  # Used for reference if outputting only the first receptor identified.
            for chain in model:
                for di in range(len(chain.xtra.get("domains", []))):
                    model.xtra["domains"][chain.xtra["domains"][di]].append((chain, di))
                    if not model.xtra["firstpair"]:
                        model.xtra["firstpair"] = [(chain, di)]  # For single domain
                ...
                for chainA, diA in model.xtra["domains"][pA]:
                    ...
                    for chainB, diB in model.xtra["domains"][pB]:
                        ...
                        if aA - aB < 25:
                            ...
                            if len(model.xtra["firstpair"]) < 2:
                                model.xtra["firstpair"] = sorted(
                                    [(chainA, diA), (chainB, diB)]
                                )
...
def Chain_switch_numbering_scheme(self, scheme, domain_index=0):
    ...
    if scheme not in [self.xtra["scheme"], "pdb"]:
        PDBNumber(
            scheme, self.xtra.get("domains", ["H", "K", "L", "A", "B", "G", "D"])
        ).numberChain(self, sequence=self.xtra.get("seqres", ""))

    if "anarci_details" in self.xtra:  # Only bother to recurse into numbered chains
        if self.xtra.get("loaded", "") == scheme + str(domain_index):
            return  # Only switch if different.
        for child in self:
            child.switch_numbering_scheme(scheme, domain_index=domain_index)
        self.child_dict = (
            {}
        )  # Reset the child dict on a separate loop to prevent conflicts
        for child in self:
...
def Residue_switch_numbering_scheme(self, scheme, domain_index=0):
    ...
    self.id = (self.id[0],) + self.xtra.get(
        scheme + str(domain_index), self.xtra.get(scheme, self.xtra["pdb"])
    )
...
def Chain_interface_cys_CA(self, scheme, domain_index=0):
    ...
    try:
        return self[(" ", inter_cys[self.xtra["domains"][domain_index]][scheme], " ")][
            "CA"
        ]
    except KeyError:
        return
...
def compile_remarks(chain, rtype, only_loaded=False):
    ...
    if only_loaded:
        r.append(
            "CHAIN %s N_%s-V-DOMAINS %d"
            % (chain.id, rtype, min(1, len(chain.xtra.get("domains", []))))
        )
    else:
        r.append(
            "CHAIN %s N_%s-V-DOMAINS %d"
            % (chain.id, rtype, len(chain.xtra.get("domains", [])))
        )

    # Create remark records about the chain types, pairings and germline assignments
    for i in range(len(chain.xtra.get("domains", []))):
        if only_loaded and i != int(chain.xtra.get("loaded", "9")[-1]):
            continue
        ...
        if chain.xtra["pairing"][i] is not None:
            r.append(
                di
                + "PAIREDWITH CHAIN %s DOMAIN %d"
                % (chain.xtra["pairing"][i][0].id, chain.xtra["pairing"][i][1])
            )
...
def split_scfv(structure):
    ...
    for model in structure:
        ...
        for chain in chains:
            if (
                len(chain.xtra.get("domains", [])) == 2
                and chain.id.lower() not in model
            ):
...
def pairwise_muscle(seq1, seq2, exact=True):
    ...
    if exact:
        p = subprocess.Popen(
            [muscle_path, "-gapopen", "-1.001", "-gapextend", "-1"],
...
def get_alignment_dict(ali1, ali2):
    ...
    for ap in range(len(ali1)):
        if ali1[ap] != "-" and ali2[ap] != "-":
            assert (
                ali1[ap] == ali2[ap]
            ), "The sequence in the structure did not match that in the provided sequence (seqres entry)"
            p1 += 1
            p2 += 1
    ...
    for residue in chain.child_list[1:]:
        try:
            if residue.id[0].strip():
                continue  # Don't consider HETATMS. Enforce them to be missed
            if (
                abs(last["C"] - residue["N"]) > 1.5
            ):  # Generous cut-off (average 1.33) for C-N peptide bond.
                return False, i
        except KeyError:
            return False, i
        last = residue
...
def get_region(position, chain, numbering_scheme="chothia", definition="chothia"):
    ...
    **Note** this function does not know about insertions on the sequence. Therefore, it will get the region annotation
    ...
    if numbering_scheme == "wolfguy" or definition == "wolfguy":
        assert (
            definition == "wolfguy" and numbering_scheme == "wolfguy"
...
def annotate_regions(
    ...
    if chain == "H":
        if (
            ...
            if (
                ins
            ):  # Add postions backwards based on the number of insertions starting with H35.
            ...
            if (
                ins
            ):  # Add postions based on the number of insertions starting with H33 for the Chothia definition. H34 for the IMGT definition.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", type=str, help="A structure to be numbered", dest="inputstructure"
    )
    parser.add_argument(
        "-o",
        type=str,
    ...
    parser.add_argument(
        ...
        "-s",
        type=str,
        choices=scheme_names + ["pdb"],
        default="imgt",
        help="Which numbering scheme should be used. i, k, c, m, w and a are shorthand for IMGT, Kabat, Chothia, Martin (Extended Chothia), Wolfguy and Aho respectively. Default IMGT. Use pdb to retain the numbering but get the annotations as remarks",
        dest="scheme",
    )
    parser.add_argument(
        "--receptor",
        ...
        choices=["ig", "tr"],
        default="ig",
        help="Choose whether to number Antibody (ig) or TCR (tr) domains.",
        dest="receptor",
    )
    parser.add_argument(
        "--rename",
        action="store_true",
        default=False,
        help="Rename the receptor chains with H and L (ig) or B and A (tr). Only receptor chains output. First pair identified used.",
        dest="rename",
    )
    parser.add_argument(
        "--fvonly",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="When they are found split single chain fvs into two seperate chains (fvonly becomes true)",
        dest="splitscfv",
    )
    parser.add_argument(
        "--warnings",
        action="store_true",
        default=False,
        help="Report warnings about missing residues",
        dest="warnings",
    )
    ...
    switchback = False
    if args.scheme == "pdb" or (
        (args.fvonly or args.splitscfv) and args.scheme in ["aho", "wolfguy"]
    ):
    ...
    if args.receptor == "ig":
        sparser = AntibodyPDBParser(
            QUIET=True, scheme=args.scheme, warnings=args.warnings
        )
    ...
    try:
        ...
        for chain in structure.get_chains():
            print(
                ...
                + "\nREMARK   6 ".join(
                    compile_remarks(
                        chain, args.receptor.upper(), only_loaded=args.splitscfv
                    )
                ),
                file=of,
            )
